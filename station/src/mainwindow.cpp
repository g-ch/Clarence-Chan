    ...
    ui(new Ui::MainWindow)
{
    ...
    */
    ...
    QObject::connect(&message,SIGNAL(global_Compass_Signal()),this,SLOT(global_Compass_Slot()));
    QObject::connect(&message,SIGNAL(global_Rel_Alt_Signal()),this,SLOT(global_Rel_Alt_Slot()));
    QObject::connect(&message,SIGNAL(local_Orientation_Signal()),this,SLOT(local_Orientation_Slot()));
    ...
    //方向、上升速度仪表设定
    ui->frame_Test->setFrameStyle(0);
    ui->frame_Test->setFixedSize(150,150);

    QPalette   palette;
    QPixmap pixmap(":/icon/Icons/1-150.png");//方向仪表背景图片
    palette.setBrush(ui->frame_Test-> backgroundRole(),QBrush(pixmap));
    ui->frame_Test->setPalette(palette);
    ui->frame_Test->setMask(pixmap.mask());  //可以将图片中透明部分显示为透明的
    ui->frame_Test->setAutoFillBackground(true);

    compass_label = new QLabel(ui->frame_Test);
    compass_label->setFixedWidth(150);
    compass_label->setFixedHeight(150);
    compass_label->move(0,0);
    ...
    velocity_label = new QLabel(ui->frame_Velocity_Z);
    velocity_label->setFixedWidth(150);
    velocity_label->setFixedHeight(150);
    velocity_label->move(0,0);
...
void MainWindow::battey_Slot()
{
    ...
    if(message.battery_voltage>20.0)
    {
        ...
        else if(message.battery_voltage>20.8)battery_status=1;
        else if(message.battery_voltage>20.4)battery_status=0;
        else battery_status=-1;
    }
    else if(message.battery_voltage>10.0)
    {
...
void MainWindow::radio_Status_Slot()
{
    ...
    //画信号图
    if(message.radio_rssi>-30)radio_status=3;
    else if(message.radio_rssi>-50)radio_status=2;
    else if(message.radio_rssi>-80)radio_status=1;
    else if(message.radio_rssi>-100)radio_status=0;
    else radio_status=-1;
...
void MainWindow::global_Velocity_Slot()
{
    ...
    painter.end();
    velocity_label->setPixmap(QPixmap::fromImage(image));//在label上显示图片
...
void MainWindow::global_Compass_Slot()
{
    ui->lineEdit_Compass->setText(QString::number(message.global_position.compass_hdg));
    //航向仪表绘图
   QPainter painter;
   ...
   painter.translate(75,75);
   painter.rotate(message.local_position.orientation.yawd);
   ...
   painter.end();
   compass_label->setPixmap(QPixmap::fromImage(image));//在label上显示图片

    //status_painter->compassd=message.global_position.compass_hdg;//飞行图下方的航向表
    //status_painter->update();//######//

    /*//用图片旋转来作航向表指针
    QPixmap pixmap(":/icon/Icons/2-83-86.png");
    QMatrix leftmatrix;
    leftmatrix.rotate(message.global_position.compass_hdg);//旋转角度,以图片中心为旋转轴
    pixmap=pixmap.transformed(leftmatrix,Qt::SmoothTransformation);
    QPalette palette;
    palette.setBrush(compass_label-> backgroundRole(),QBrush(pixmap));
    compass_label->setPalette(palette);
    compass_label->setAutoFillBackground(true);
    */
}
...
void MainWindow::local_Orientation_Slot()
{
    //去抖动
