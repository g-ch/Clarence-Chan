...
void chatterCallback_GlobalPosition_CompassHdg(const std_msgs::Float64 &msg);
...
void chatterCallback_GlobalPosition_CompassHdg(const std_msgs::Float64 &msg)//地磁偏角
{
    //cout<<"P"<<msg.data<<endl;
    message.global_position.compass_hdg=msg.data;
    message.msg_Send_Compass();
}
...
void MavrosMessage::run()
{
    ...
    ros::Subscriber sub5 = n.subscribe("/mavros/global_position/compass_hdg", 1000,chatterCallback_GlobalPosition_CompassHdg);
    ros::Subscriber sub6 = n.subscribe("/mavros/global_position/gp_vel", 1000,chatterCallback_GlobalPosition_GpVel);
    ros::Subscriber sub7 = n.subscribe("/mavros/local_position/local", 1000,chatterCallback_LocalPosition_Local);
    ros::Subscriber sub8 = n.subscribe("/mavros/wind_estimation",1000,chatterCallback_WindEstimation);
    ros::Subscriber sub9 = n.subscribe("/mavros/battery",1000,chatterCallback_Battery);
    ros::Subscriber sub10 = n.subscribe("/mavros/radio_status",200,chatterCallback_RadioStatus);
    ros::Subscriber sub11 = n.subscribe("/mavros/gps/fix",100,chatterCallback_GPS_Fix);
    ros::Subscriber sub12 =n.subscribe("/mavros/px4flow/raw/optical_flow_rad",1000,chatterCallback_Optical_Flow);
    ros::Subscriber sub13 =n.subscribe("/mavros/imu/temperature",100,chatterCallback_Imu_Temperature);
    ros::Subscriber sub14 =n.subscribe("/mavros/time_reference",500,chatterCallback_Time);
    //循环执行
    ros::spin();
}
